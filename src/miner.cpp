...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("STOTINKAMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("STOTINKAMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("STOTINKAMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("stotinka-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running STOTINKAMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
